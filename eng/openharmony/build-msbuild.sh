...
cd -- "$msbuild_source"
msbuild_commit=${DOTNET_OHOS_SOURCE_COMMIT:-$(git rev-parse HEAD)}
[[ $msbuild_commit =~ ^[0-9a-f]{40}$ ]] || { echo 'Missing source commit provenance' >&2; exit 2; }
...
exec bash build.sh -configuration Release -pack -v quiet /m:1 \
    ...
    "/p:RepositoryCommit=$msbuild_commit" "/p:SourceRevisionId=$msbuild_commit" \
    /p:RepositoryUrl=https://github.com/oheco/dotnet-sdk \
